...
module.exports = function(house, device) {
	...
	this.poll = function(cb){
		api.getSwitchState(properties).then(function (state) {
			this.status.on = state;
    		if(typeof cb === 'function'){cb(this.status);}
		}.bind(this)).catch(function(err){
			house.log.warning("Failed to get switch state for " + this.status.name + "\r" + err);
